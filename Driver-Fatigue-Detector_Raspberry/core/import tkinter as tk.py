...
class FatigueDetectionGUI:
    """Thin wrapper around legacy start_gui procedure."""
    def __init__(self, detector):
        self.detector = detector  # reserved if future direct frame pull needed

    def run(self):
        # reuse existing start_gui workflow (blocks with mainloop)
        start_gui()

    def cleanup(self):
        # call existing on_closing to release resources
        try:
            on_closing()
...
def start_gui():
    """Build and start the Tkinter GUI main loop."""
    ...
    # INIT camera source 0 automatically
    try:
        vs = VideoStream(src=0).start()
        time.sleep(2.0)
        test_frame = vs.read()
        camera_available = test_frame is not None
        print(f"[Camera] Auto-init source 0 -> {'OK' if camera_available else 'FAILED'}")
    except Exception as e:
        print(f"[Camera] ERROR auto-init 0: {e}")
        camera_available = False
